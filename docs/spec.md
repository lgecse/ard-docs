...
To guarantee that any system can participate in discovery regardless of its execution stack, a discovery service **MUST** expose a standard HTTP REST search interface. While specialized protocols may be used for execution, discovery requires a universal baseline that any HTTP client can access.
...
```
{
  ...
  "entries": [
    ...
    },
    {
      ...
      "displayName": "Acme Global Discovery Service",
...
| representativeQueries | Array | Sample natural-language queries (e.g., \["find me a flight booking agent"\]). Used by discovery services to build semantic vector embeddings for search ranking. SHOULD contain 2–5 examples. |
...
2. **Dynamic Discovery**: Active, searchable discovery services that index static catalogs and expose dynamic search endpoints.
...
* **DNS**: Publishing Service Binding records in the DNS that point directly to either a static capability manifest (e.g., \_catalog.\_agents.example.com) or a dynamic discovery service search endpoint (e.g., \_search.\_agents.example.com).
...
The response returns standard catalog entries with additional relevance scores, plus optional referrals. The score parameter denotes **semantic relevance ranking** (0-100) computed by the discovery service, indicating how well the entry satisfies the natural language query criteria. It is strictly an informational relevance metric and MUST NOT be interpreted by orchestrators as a cryptographic trust, compliance, or safety rating. Trust evaluation is fully decoupled and handled independently via the verification procedures in the trustManifest layer.
  ...
  "referrals": [
    {
      ...
      "displayName": "Public Discovery Service",
...
While this specification mandates the REST interface for interoperability, implementations may employ advanced techniques to resolve natural language queries to specific agent endpoints. An example flow, drawing from research on Agent Naming Services (ANS) and Federated discovery services, involves the following steps:

1. **Semantic Translation & Embedding**:  
   * **LLM Query Interpretation**: The discovery service uses an LLM to extract specific multi-dimensional requirements from the natural language text field, translating it into structured capability attributes (e.g., domain: travel, skill: flight\_booking, constraints: meal\_preference).  
   * **Vector Embeddings**: The discovery service may also convert the query description into a dense vector embedding to understand semantic meaning (e.g., matching "foreign exchange" to "forex" or "international money transfer").  
2. **Global Discovery via Federated Routing**:  
   * Advanced implementations may execute this query against a federated network. For example, using semantic attributes or embedding vectors to perform a search across a Distributed Hash Table (DHT) (e.g., an extended IPFS Kademlia DHT) or by leveraging **DNS-AID** to discover authoritative discovery services for specific domains.  
...
While the REST API is mandated as the floor for interoperability, a discovery service **MAY** additionally expose its search capability natively via an MCP Tool or an A2A Skill to preserve native orchestrator flows.
...
Because the REST API is mandated, service-to-service routing (federation) becomes a simple HTTP operation. The client controls federation through the federation query parameter:

* **auto**: The discovery service queries upstream discovery services automatically, merges their results with its own, and returns a unified response. The client gets a single merged result set.  
* **referrals**: The discovery service returns its results plus catalog entries for other discovery services the client may query. The client decides which to follow.  
* **none**: The discovery service searches only its own index.
...
```
{
  ...
  "referrals": [
    ...
    },
    {
      ...
      "displayName": "Travel Discovery Service",
...
1. The orchestrator queries the enterprise discovery service with federation: "referrals".  
2. The discovery service returns an internal expense agent, plus referrals to other discovery services.  
3. The orchestrator follows a referral to a public discovery service and queries it for flight booking agents.  
